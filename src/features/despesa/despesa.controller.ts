  ...
  public async cadastrar(req: Request, res: Response) {
    ...
    try {
      ...
      if (error instanceof Error) {
        ...
        if (error.message === 'ACESSO_NEGADO') return res.status(403).json({ error: 'Acesso negado! Você não tem permissão para cadastrar despesas nesta propriedade.' });
  ...
  public async buscarPorId(req: Request, res: Response) {
    ...
    try {
      ...
      if (error instanceof Error) {
        ...
        if (error.message === 'ACESSO_NEGADO') return res.status(403).json({ error: 'Acesso negado ao buscar despesa' });
  ...
  public async listarPorPropriedade(req: Request, res: Response) {
    ...
    try {
      ...
      if (error instanceof Error) {
        ...
        if (error.message === 'PROPRIEDADE_NAO_ENCONTRADA') return res.status(404).json({ error: "Propriedade não encontrada" });
        if (error.message === 'ACESSO_NEGADO') return res.status(403).json({ error: 'Acesso negado ao listar despesas da propriedade' });
